...
Page({
  ...
  comeSetting:function(e){
    this.setData({
      isSetting2: true, state: 1, picker1Value: 0, picker2Value: 0, picker3Value: 0, dateValue: '请选择日期',timeValue: '请选择时间',})
      console.log(this.data.isSetting2,this.data.state)
  },
  //点击去程设定
  returnSetting:function(e){
    this.setData({ isSetting2: true, state: 2,picker1Value: 0, picker2Value: 0, picker3Value: 0, dateValue: '请选择日期', timeValue: '请选择时间',})
  },
  close:function(e){
    this.setData({isSetting2:false})
  },
  register:function(e){
    ...
    ajax.query({
      ...
      callback: function (res) {
        ...
        if (res.Table.length != 0) {
          ...
          that.setData({ isSetting2: true,state:1, picker1Value: res.Table[0].peopleCount, picker2Value:d, picker3Value: res.Table[0].pointId, dateValue: res.Table[0].arriveTime.substring(0, 10), timeValue: res.Table[0].arriveTime.substring(11, 16)})
        }
      }
    })
  },
  modifygo(e){
    ...
    ajax.query({
      ...
      callback: function (res) {
        console.log(res)
    that.setData({ isSetting2: true, state: 2, picker1Value: res.Table[0].peopleCount,  picker3Value: res.Table[0].pointId, dateValue: res.Table[0].arriveTime.substring(0, 10), timeValue: res.Table[0].arriveTime.substring(11, 16)  })
      }
