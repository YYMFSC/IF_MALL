...
public class DoorHandler : BaseHandler
{
    ...
    public void Remove(HttpContext context)
    {
        ...
        try
        {
            ...
            {
                mall.DoorToDoor.Remove(mall.DoorToDoor.Find(int.Parse(item)));//删除
                mall.SaveChanges();
            }
    ...
    public void GetList(HttpContext context)
    {
        ...
        //在查询筛选中加条件
        Expression<Func<DoorToDoor, bool>> seleWhere = o => true;//o.n_state == (int)Enum_BasicInfoStatus.Enable ;
        seleWhere = seleWhere.And(o => o.adress.Contains(unitkey));

        var linq = from v in mall.Set<DoorToDoor>()
                   select v;
      ...
      public void EditKind(HttpContext context)
    {
        ...
        try
        {
            DoorToDoor new1 = mall.DoorToDoor.Find(int.Parse(iddf.Value.ToString()));
            new1.kind =int.Parse(kind.Value.ToString());
            //使用add方法把模型加进去
            mall.DoorToDoor.Attach(new1);
